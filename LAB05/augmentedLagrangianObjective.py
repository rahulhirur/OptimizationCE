...
class augmentedLagrangianObjective:
    ...
    def objective(self, x: np.array):
        myObjective = self.f.objective(x) + self.alpha * np.sum(self.h.objective(x)) + 0.5*self.gamma*np.sum(np.power(self.h.objective(x),2))

        return myObjective

    def gradient(self, x: np.array):
        myGradient = self.f.gradient(x) + np.sum((self.alpha+self.gamma*self.h.objective(x))) *self.h.gradient(x)
